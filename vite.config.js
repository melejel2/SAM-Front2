...
function appVersionPlugin() {
    var buildId = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    var releasedAt = new Date().toISOString();
    var releaseNotesPath = path.resolve(path.resolve(), "scripts", "release-notes.txt");
    var releaseNotes = existsSync(releaseNotesPath)
        ? readFileSync(releaseNotesPath, "utf-8").trim()
        : "What's new in v".concat(pkg.version);
    return {
        name: "app-version",
        apply: "build",
        ...
        transformIndexHtml: function () {
            return [
                {
                    tag: "script",
                    children: "window.__APP_VERSION__ = \"".concat(buildId, "\"; window.__APP_PKG_VERSION_EMBEDDED__ = \"").concat(pkg.version, "\";"),
                    injectTo: "head",
                },
            ];
        },
