...
export const EditBOQEditingSection: React.FC<BOQEditingSectionProps> = memo(({
    ...
    const { formData, setFormData, allBuildings } = useEditWizardContext();
    ...
    const displayItems = useMemo((): DisplayBOQItem[] => {
        ...
        const buildingBOQ = formData.boqData.find(
            (b: any) => b.buildingId === activeTabInfo.buildingId && b.sheetName === activeTabInfo.tradeName
        );
        const items: DisplayBOQItem[] = (buildingBOQ?.items || []).map((item: any) => ({
            ...item,
            _isEmptyRow: false
        }));
    ...
    const addNewBOQItem = useCallback((initialData: Partial<BOQItem>) => {
        ...
        if (buildingIndex >= 0) {
            ...
            const building = allBuildings.find((b: any) => b.id === activeTabInfo.buildingId);
            updatedBOQData.push({
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                const existingDataMap = new Map(
                    formData.boqData.map((item: any) => [`${item.buildingId}-${item.sheetName}`, item])
                );
    ...
    return (
        <div className="bg-base-100 border border-base-300 rounded-lg h-full flex flex-col overflow-hidden">
            ...
            <BOQImportModal
                ...
                contractDataSetId={0}
                availableBuildings={allBuildings.map((building: any) => ({
                    id: building.id,
                    name: building.name,
                    sheets: building.sheets.map((sheet: any) => ({
                        id: sheet.id,
                        name: sheet.name
                    }))
                }))}
